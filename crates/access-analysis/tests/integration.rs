...
fn run_shadow_access_fixture(mode: CompilationMode) {
    let is_debug = mode == CompilationMode::Debug;
    let sets = analyze_named("sdk-test-shadow-access", mode);

    if is_debug {
        // Debug: helpers are not inlined, so sets are exact and minimal.
        assert_reads_writes(&sets["write_a"], &[], &["a"]);
        assert_reads_writes(&sets["write_b"], &[], &["b"]);
        assert_reads_writes(&sets["write_c_1"], &[], &["c"]);
        assert_reads_writes(&sets["write_c_2"], &[], &["c"]);
        assert_reads_writes(&sets["read_c"], &["c"], &[]);
        assert_reads_writes(&sets["read_d_1"], &["d"], &[]);
        assert_reads_writes(&sets["read_d_2"], &["d"], &[]);
    } else {
        // Release: wasm-opt inlining may over-approximate soundly; check required
        // tables are present (or wildcard covers them) rather than exact equality.
        let universe_ab = &["a", "b"];
        let universe_c = &["c"];
        let universe_d = &["d"];
        let sound_or_wildcard = |set: &AccessSet, writes: &[&str], universe: &[&str]| {
            if !set.wildcard {
                assert_sound_superset(set, &[], writes, universe);
            }
        };
        sound_or_wildcard(&sets["write_a"], &["a"], universe_ab);
        sound_or_wildcard(&sets["write_b"], &["b"], universe_ab);
        sound_or_wildcard(&sets["write_c_1"], &["c"], universe_c);
        sound_or_wildcard(&sets["write_c_2"], &["c"], universe_c);
        // read_c reads c
        if !sets["read_c"].wildcard {
            assert_sound_superset(&sets["read_c"], &["c"], &[], universe_c);
        }
        if !sets["read_d_1"].wildcard {
            assert_sound_superset(&sets["read_d_1"], &["d"], &[], universe_d);
        }
        if !sets["read_d_2"].wildcard {
            assert_sound_superset(&sets["read_d_2"], &["d"], &[], universe_d);
        }
    }

    // heavy_a/heavy_b: both read and write the same table (both modes)
    // Wildcard is always sound; non-wildcard must contain both a read and write of the table.
    if !sets["heavy_a"].wildcard {
        assert_sound_superset(&sets["heavy_a"], &["a"], &["a"], &["a"]);
    }
    if !sets["heavy_b"].wildcard {
        assert_sound_superset(&sets["heavy_b"], &["b"], &["b"], &["b"]);
    }

    // indirect_touch: the fn-pointer array prevents devirtualization in Debug.
    // Release mode may inline/devirtualize, converting the call_indirect to direct
    // calls (both touch_a and touch_b then appear in the closure), which the
    // analyzer attributes soundly without needing wildcard. So the wildcard
    // assertion is Debug-only; Release only requires soundness (no under-approx).
    if is_debug {
        assert!(
            sets["indirect_touch"].wildcard,
            "indirect_touch must be wildcard in Debug (fn-ptr array defeats devirtualization): {:?}",
            sets["indirect_touch"]
        );
    } else {
        // Release: either wildcard OR soundly over-approximate (both a and b covered).
        let s = &sets["indirect_touch"];
        if !s.wildcard {
            // Must cover both tables since both branches are reachable.
            assert_sound_superset(s, &[], &["a", "b"], &["a", "b"]);
        }
        // If wildcard, no further assertion needed — wildcard is always sound.
    }
}

#[test]
fn shadow_access_fixture_debug() {
    run_shadow_access_fixture(CompilationMode::Debug);
}

#[test]
fn shadow_access_fixture_release() {
    run_shadow_access_fixture(CompilationMode::Release);
}
